...
def train_multimodal_model(config, is_full_train=False, experiment_logger=None):
    ...
    if is_full_train:
        ...
        ge_training_folds, ge_validation_folds = load_ge_train_test_folds(gene_expression_df, split_dict)
        wsi_training_folds, wsi_validation_folds = load_folds(wsi_features, split_dict, is_cv=False, ignore_missing=True)
        n_folds = 1
    else:
        logger.info(f"Using {len(split_dict['CV'])} cross-validation folds")
        ge_training_folds, ge_validation_folds = load_ge_cv_folds(gene_expression_df, split_dict)
        wsi_training_folds, wsi_validation_folds = load_folds(wsi_features, split_dict, is_cv=True, ignore_missing=True)
